...
class MyEngine(Engine):
    ...
    def train(engine, mini_batch):
        ...
        x, y = mini_batch  ## |x| = (bs, 784)
        x, y = x.to(engine.device), y.to(engine.device)

        # Take feed-forward
        y_hat = engine.model(x) ## |y_hat| = (bs, 10)
        ...
        if isinstance(y, torch.LongTensor) or isinstance(y, torch.cuda.LongTensor): ## if classification이면
            accuracy = (torch.argmax(y_hat, dim=-1) == y).sum() / float(y.size(0))
        else:
            accuracy = 0
        # 벡터의 크기(l2 norm)
        p_norm = float(get_parameter_norm(engine.model.parameters()))  # 커진다 -> 학습이 진행되고있다
        g_norm = float(get_grad_norm(engine.model.parameters())) # 작아진다 -> loss가 최소값에 다가가고 있다
...
class Trainer():
    ...
    def train(
        ...
        # train 한 epoch 끝날때마다 이 함수를 부를것이다
        # validation_engine을 한 epoch만 수행한다
        def run_validation(engine, validation_engine, valid_loader):
            validation_engine.run(valid_loader, max_epochs=1)


        # train_engine에서 한 epoch이 끝나면 run_validation 실행해라!!
        ...
        # validation_engine에서 한 epoch이 끝나면 check_best를 실행해라!!
        validation_engine.add_event_handler(
            Events.EPOCH_COMPLETED, # event
            MyEngine.check_best, # function
        )

        # validation_engine에서 한 epoch이 끝나면 save_model을 실행해라!!
        validation_engine.add_event_handler(
            Events.EPOCH_COMPLETED, # event
        ...
        # train_engine을 n epochs 만큼 실행해라
        train_engine.run(
            train_loader,
