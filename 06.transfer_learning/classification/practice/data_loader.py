...
from torchvision import data_transforms
...
def get_loaders(config, input_size):
    ...
    dataset_name = condig.dataset_name
    ...
    train_set, valid_set, test_set = divide_dataset(
        ...
        config.train_ratio, condif.valid_ratio, config.test_ratio
    )
...
def divide_dataset(
    train_set, test_set, data_name='catdog',
    train_ratio=.6, valid_ratio=.2, test_ratio=.2
):
    if data_name == 'catdog':
        train_cnt = int(len(train_set) * train_ratio)
        valid_cnt = int(len(train_set) * valid_ratio)
        test_cnt = len(train_set) - train_cnt - valild_cnt

        train_set, valid_set, test_set = random_split(
            train_set, [train_cnt, valid_cnt, test_cnt]
        )
    elif data_name == 'hymenoptera':
        valid_ratio = valid_ratio / (train_ratio + valid_ratio) # 0.25
        valid_cnt = int(len(train_set) * valid_ratio)
